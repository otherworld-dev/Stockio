...
Each bot has its own LLM advisor (`llm_advisor.py`) that can veto trades based on market regime assessment. The advisor sees shadow outcomes (vetoed trades that would have won/lost) as feedback. Both the LLM scorer and advisor use `llm_strategy_model` (Opus 4.8) for better reasoning, while sentiment stays on `llm_model` (Haiku) to keep costs low. The advisor is only called on cycles with pending signals — no-signal cycles skip the API call entirely. Both are self-adapting: they receive their own performance stats (win rate, P&L by instrument, losing streaks) in each prompt.
