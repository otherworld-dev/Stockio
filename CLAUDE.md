...
Each bot has its own LLM advisor (`llm_advisor.py`) that can veto trades based on market regime assessment. The advisor sees shadow outcomes (vetoed trades that would have won/lost) as feedback. Models are tiered by volume: the LLM scorer (1 bot, decides trades) uses `llm_strategy_model` (Opus 4.8) where reasoning matters; the veto advisor runs on all 5 bots every cycle so it uses the cheap `llm_advisor_model` (Haiku); sentiment uses `llm_model` (Haiku). The advisor is only called on cycles with pending signals — no-signal cycles skip the API call entirely. Both are self-adapting: they receive their own performance stats (win rate, P&L by instrument, losing streaks) in each prompt.
